...
def _format_draft(owner: ServiceOwner) -> str:
    ...
    lines += [
        ...
        f"Telegram: {e(owner.draft_telegram or '(не указано)')}",
        f"Рабочие дни: {e((owner.draft_working_days or '').replace(',', ', ') or '(не выбрано)')}",
        f"Время работы: {owner.draft_open_time or '?'}-{owner.draft_close_time or '?'}",
        f"Диагностика: {int(owner.draft_diagnostics_price) if owner.draft_diagnostics_price else 0} руб.",
...
async def cmd_start(message: types.Message, state: FSMContext) -> None:
    ...
    if owner is None:
        await message.answer(
            "Добро пожаловать в ESAS Partner!\n"
            "Здесь вы можете зарегистрировать свой сервисный центр "
            "и управлять заявками.",
...
async def show_status(message: types.Message) -> None:
    ...
    lines = [
        ...
        f"Рабочие дни: {(svc.working_days or '').replace(',', ', ') or '-'}",
    ...
    if svc.upgrade_categories:
        lines.append(f"Категории апгрейда: {svc.upgrade_categories.replace(',', ', ')}")
    await message.answer("\n".join(lines))


@router.message(F.text == "Техподдержка")
async def cmd_support(message: types.Message, state: FSMContext) -> None:
    from bot.core.config import SUPPORT_USER
