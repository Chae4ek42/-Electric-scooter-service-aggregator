...
Дополнительно исключаются незаконченные анкеты партнёров: `telegram_id IS NULL OR status = 'активный'`. Сервисы из Google Sheets (без `telegram_id`) всегда попадают в выборку.
