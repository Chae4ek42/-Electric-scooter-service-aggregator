...
> **Модель данных:** Сервисы и партнёры — это одна и та же сущность. Все данные хранятся в единой таблице `services`. Draft-поля, telegram_id, статус владельца и банковские реквизиты — поля `Service`. Таблицы `service_owners` не существует. Незаконченные анкеты (`registration_complete = False`) не отображаются в списке сервисов при ранжировании и не индексируются в административных панелях обоих ботов.
...
/start
  └─► (уже зарегистрирован?) ──Yes──► главное меню
       ...
       (если Ремонт или Комплекс)
        reg_category          ← inline: [Электрика / Механика / Электрика + механика]
            ▼
       (если Апгрейд)
        ...
        reg_diag_included     ← inline: [Входит в стоимость / Оплачивается отдельно]
            ▼
        reg_confirm           ← карточка с итоговыми данными + [Отправить / Заново / Заполнить банк. реквизиты]
            │                    Банковские реквизиты не обязательны для отправки
            │
            ├─ «Заполнить банк. реквизиты» / «Изменить реквизиты» →
...
- **Кнопка «Заполнить банк. реквизиты» / «Изменить реквизиты»** на экране подтверждения — запускает поток `reg:fill_bank` (legal_form → tax_system → bank_details). Если legal_form или tax_system уже заполнены, соответствующие шаги пропускаются.
...
- **Редактировать профиль** — выбор операционного поля для изменения (название, адрес, телефон и др.)
- **Банковские реквизиты** — просмотр и редактирование реквизитов (форма, налогообложение, банк-счёт и др.)
   ...
   └──► start_work        ← партнёр нажал «Принять в работу»
           ...
           └─ estimate_confirm     ← подтверждение сметы
            ...
            in_progress       ← клиент получает смету, подтверждает/отклоняет
                  │
                  ├─► update_price_cost    ← партнёр нажал «Изменить цену»
                  │       ▼
                  │   update_price_reason  ← ввод причины (мин. 5 симв.)
                  │       ▼
                  │   total_cost обновляется, клиенту уведомление
                  │
                  ▼
...
| Изменить цену              | `in_progress`             | FSM: новая цена → причина; `total_cost` + `price_change_reason` + `price_updated_at` сохраняются; клиенту уведомление со старой ценой, новой ценой и причиной |
...
Разделено на два раздела: «Редактировать профиль» (операционные поля) и «Банковские реквизиты» (финансовые поля).

### 6.1 Редактирование профиля (`pedit:`)
...
Быстрый переключатель статуса **не вызывает** ре-модерацию. Доступен через кнопку «Статус сервиса» в главном меню.

**Поля «Редактировать профиль»:**
...
### 6.1b Редактирование банковских реквизитов (`bedit:`)

Доступно через кнопку **«Банковские реквизиты»** в главном меню. Показывает текущие значения всех 8 полей и inline-кнопки редактирования. Изменения **не вызывают** ре-модерацию.

| Поле            | callback             | Тип ввода                         |
| --------------- | -------------------- | --------------------------------- |
| Правовая форма  | `bedit:legal_form`   | inline (ИП / Юр. лицо)            |
| Налогообложение | `bedit:tax_system`   | inline (ОСНО/УСН/АУСН/Патент/НПД) |
| Расч. счёт      | `bedit:bank_account` | текст (BankAccountInput)          |
| Банк            | `bedit:bank_name`    | текст (BankNameInput)             |
| БИК             | `bedit:bik`          | текст (BikInput)                  |
| Корр. счёт      | `bedit:corr_account` | текст (CorrAccountInput)          |
| Организация     | `bedit:org_name`     | текст (OrgNameInput)              |
| ИНН             | `bedit:inn`          | текст (InnInput)                  |

После сохранения любого поля партнёр возвращается на экран «Банковские реквизиты» (`_format_bank_details` + `bank_edit_fields_kb`).
...
| Список партнёров  | `padm:partners:{page}`                            | Пагинация (10), сортировка по дате. Только анкеты с `registration_complete = True` (т.е. партнёр отправил форму на модерацию). Черновики скрыты. Заголовок: `Все партнёры: N (стр. X/Y)` |
...
> **Фильтрация незавершённых анкет:** Оба admin-модуля (`bot/handlers/admin.py` и `partner_bot/handlers/admin.py`) используют условие `Service.registration_complete.is_(True)` во всех запросах, связанных с партнёрами. Незаполненные черновики не влияют на счётчики и не отображаются в списке.
>
> **Блокировка отправки:** В `reg_submit` проверяется `_draft_complete(owner)` перед записью статуса «ожидает». Партнёр не может отправить анкету на модерацию, пока все обязательные поля не заполнены.
