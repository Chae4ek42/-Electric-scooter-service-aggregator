...
- **Мой статус** — карточка сервиса с текущими данными (включая `upgrade_categories`, `working_days`). Тип услуг отображается на русском («Ремонт»/«Апгрейд»). Рабочие дни сортируются Пн→Вс.
...
| Список партнёров | `padm:partners:{page}` | Пагинация (10), сортировка по дате. Заголовок: `Все партнёры: N (стр. X/Y)` |
...
├── handlers/
│   ├── common.py                 (/start, /admin, /client, меню, Поддержка, Моя анкета, Мой статус)
