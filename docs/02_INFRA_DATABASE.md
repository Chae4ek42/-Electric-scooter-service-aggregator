...
> **Часовой пояс:** обоим сервисам (`client-bot`, `partner-bot`) в `docker-compose.yaml` задан `TZ=Europe/Moscow`.  
> Все временны́е метки в логах отображаются в московском времени (UTC+3).  
> Логи Redis и системы — остаются в UTC (они не используют переменную `TZ` из compose).
